...
- [x] **AGNT-01**: LangGraph StateGraph workflow with Analyze → Plan → Execute → Respond nodes
- [x] **AGNT-02**: AgentState TypedDict with messages (add_messages reducer), plan, scratchpad fields
- [x] **AGNT-03**: PostgresSaver checkpointer for conversation state persistence and resumability
...
- [x] **MEM-01**: Short-term memory using Redis (last N messages per conversation)
- [x] **MEM-02**: Context injection in Analyze node from short-term memory
- [x] **MEM-03**: LangGraph Store integration for cross-thread semantic search (long-term memory)
...
- [x] **SKIL-03**: Docker-based sandbox executor with resource limits and timeout enforcement
- [ ] **SKIL-04**: Skill lifecycle management (upload, validate, enable, disable, hot-update)
- [x] **SKIL-05**: Skill tool registration into unified Tool Registry
